...
class LyricManager {
  ...
  private cleanTTMLTranslations(
    ...
    const lang_counter = (ttml_text: string) => {
      // 不使用 lookbehind（Android WebView 兼容性问题）
      // 改为匹配 <span 或 <translation 标签内的 xml:lang 属性
      const tagRegex = /<(?:span|translation)(?:\s[^>]*)\sxml:lang="([^"]+)"[^>]*>/g;
      const langSet = new Set<string>();
      let m: RegExpExecArray | null;
      while ((m = tagRegex.exec(ttml_text)) !== null) {
        if (m[1]) langSet.add(m[1]);
      }
  ...
  private setFinalLyric(lyricData: SongLyric, req: number) {
    ...
    if (settingStore.shieldTraditionalChineseTranslation) {
      ...
      filterRedundant(lyricData.lrcData);
      filterRedundant(lyricData.yrcData);
    }
    ...
    for (const dir of localLyricPath) {
      ...
      if (isAndroid) {
        try {
          ...
          console.error(`[SPlayer] 安卓歌词匹配失败:`, e);
        }
      } else {
        // 电脑端逻辑保持不变
        // ... (原有的 searchDir 逻辑)

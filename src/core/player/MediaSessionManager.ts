...
class MediaSessionManager {
  ...
  public init() {
    ...
    // Web API 初始化 (在 Android/Browser 上必须始终执行)
    if ("mediaSession" in navigator) {
      ...
      // 快进/快退 (默认10秒或提供的时间)
      nav.setActionHandler("seekforward", (e) => {
        const skipTime = e.seekOffset || 10;
        player.seekBy(skipTime * 1000);
      });
      nav.setActionHandler("seekbackward", (e) => {
        const skipTime = e.seekOffset || 10;
        player.seekBy(-skipTime * 1000);
      });
    }

    // Android/Tauri workaround init
    if ("mediaSession" in navigator && !isElectron) {
      this.initSilentAudio();
    }
  }

  /**
   * 初始化静音音频 (用于激活 Android WebView 的 MediaSession)
   */

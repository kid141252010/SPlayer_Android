...
export const updateUserData = async () => {
  try {
    ...
    if (!userAccountRes || !userAccountRes.profile) {
      throw new Error("Failed to fetch user account profile");
    }
    const { profile } = userAccountRes;
    ...
    if (!subcountData) {
      throw new Error("Failed to fetch user subcount");
    }
    ...
    // 若部分失败
    const rejectedResults = allUserLikeResult.filter((result) => result.status === "rejected");
    if (rejectedResults.length > 0) {
      console.warn("Some user data updates failed:", rejectedResults);
      // Optional: throw only if critical ones failed, for now keep original logic but with more info
      // throw new Error("Failed to update some user data");
    }
  } catch (error) {
...
export const updateDailySongsData = async (refresh = false) => {
  try {
    ...
    if (!result || !result.data || !result.data.dailySongs) {
      console.warn("Daily recommend response is invalid or empty");
      return;
    }

...
export const useInit = () => {
  ...
  onMounted(async () => {
    ...
    // 初始化播放器
    // 移动端/Tauri 环境下禁止启动自动播放（避免系统限制和用户体验问题）
    const shouldAutoPlay = isTauri ? false : settingStore.autoPlay;
    player.playSong({
    ...
    // 在 Tauri/移动端环境下，歌词显示时保持屏幕常亮
    if (isTauri && "wakeLock" in navigator) {
      ...
      const releaseWakeLock = async () => {
        try {
          ...
          console.warn("[WakeLock] 释放失败:", e);
        }
      };

      // 当全屏播放器可见且正在播放时，开启屏幕常亮
      watch(
        () => [statusStore.showFullPlayer, statusStore.playStatus],
        ([showPlayer, playing]) => {

...
const handleTouchEnd = (e: TouchEvent) => {
  ...
  // 如果手指按下和松开的误差小于 10 像素，我们就认定这是一次真正的“点击(Tap)”
  if (Math.abs(touchEndY - touchStartY) < 10 && Math.abs(touchEndX - touchStartX) < 10) {
    const target = e.target as HTMLElement;
    if (target) {
      // 强行伪造一个原生的 MouseEvent 点击事件
      const clickEvent = new MouseEvent("click", {
        bubbles: true,
        cancelable: true,
        view: window,
      });
      // 稍微延迟 50 毫秒派发，避开底层滚动锁的干扰
      setTimeout(() => {
...
<style scoped>
.mobile-lyric-wrapper {
  /* 干掉安卓点击时产生的灰色高亮底色 */
  -webkit-tap-highlight-color: transparent;
}

/* 强制给内部所有元素加上指针样式，修复 WebView 漏接事件的祖传 Bug */
:deep(*) {
  cursor: pointer;
}
</style>
